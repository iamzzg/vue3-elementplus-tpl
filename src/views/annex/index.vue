  ...
  <div class="annex">
    <div class="page">
      ...
      <el-form
        ...
        <transition name="fade-top" mode="out-in">
          <section v-if="showObj.show2">
            <p class="paragraph">
              二、（地面层、高架层表述）项目拟用地总面积
              <el-form-item>
                <el-input></el-input>
              </el-form-item>
              公顷，其中农用地**公顷（耕地**公顷，园地**公顷，林地**公顷，其他农用地**公顷），建设用地**公顷，未利用地**公顷，不涉及围填海，不占用基本农田。
            </p>
            <p class="paragraph">
...
<script setup>
  ...
  const { transitionName } = appSetting
  ...
  const onSubmit = () => {
    ...
    // showObj.value.show1 = !showObj.value.show1
  }

  const showNext = (() => {
    let nextIndex = 2
    return () => {
      showObj.value[`show${nextIndex}`] = true
      nextIndex = 3
      return nextIndex
    }
  })()
